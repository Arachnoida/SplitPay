...
export default function CustomerWorkspace({
  ...
  const handlePayWithMidtrans = async (
    ...
    const newLog = `[${new Date().toLocaleTimeString()}] Creating Midtrans Snap transaction -> Name: ${name}, Rp ${shareAmount.toLocaleString("id-ID")}...`;
    setWebhookLogs((prev) => [newLog, ...prev]);
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const detailMessage =
          data?.detail?.error_messages?.join(", ") ||
          data?.detail?.validation_messages?.join(", ") ||
          data?.detail?.message ||
          data?.error ||
      ...
      const successLog = `[${new Date().toLocaleTimeString()}] Midtrans Snap ready. Order ID: ${data.orderId}. Membuka halaman pembayaran sandbox...`;
      setWebhookLogs((prev) => [successLog, ...prev]);
      ...
      const errorLog = `[${new Date().toLocaleTimeString()}] Midtrans Error: ${err.message || "Gagal membuka pembayaran."}`;
      setWebhookLogs((prev) => [errorLog, ...prev]);
      setErrorText(err.message || "Gagal membuka pembayaran Midtrans.");
    } finally {
      setSimulatings((prev) => ({ ...prev, [contributorId]: false }));
    }
  };
                        ...
                        {c.paymentStatus === "PAID" ? (
                          ...
                          <div className="mt-4 pt-3 border-t border-slate-100 space-y-3">
                            ...
                            <div className="grid grid-cols-1 gap-2">
                              <button
                                type="button"
                                onClick={() => handlePayWithMidtrans(c.id, c.name, c.shareAmount)}
                                disabled={simulatings[c.id]}
                                className="w-full py-2 bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs rounded-xl shadow-xs transition-colors cursor-pointer flex items-center justify-center gap-2 disabled:opacity-60"
                              >
                                {simulatings[c.id] ? (
                                  <>
                              ...
                              <button
                                ...
                                disabled={simulatings[c.id]}
                                className="w-full py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs rounded-xl shadow-xs transition-colors cursor-pointer flex items-center justify-center gap-2 disabled:opacity-60"
                              >
                                {simulatings[c.id] ? (
                                  <>
                                    <RefreshCw className="w-3.5 h-3.5 animate-spin" />
